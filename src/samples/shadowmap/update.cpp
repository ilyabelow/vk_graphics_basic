...
void SimpleShadowmapRender::UpdateUniformBuffer(float a_time)
{
  ...
  m_uniforms.lightDirection = m_light.cam.forward();
  m_uniforms.lightInnerCos  = cos(lightInnerAngle * DEG_TO_RAD);
  m_uniforms.lightOuterCos = cos(lightOuterAngle * DEG_TO_RAD);
